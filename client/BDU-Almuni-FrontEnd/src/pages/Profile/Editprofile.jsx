...
const Editprofile = () => {
  ...
  const [showEditpersonalinfoPopup, setshowEditpersonalinfoPopup] =
    useState("");
  ...
  useEffect(() => {
    if (!username || !userId) return; // Prevent fetching if username or userId is null

    const getPosts = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}/posts/${username}`,
          {
            credentials: "include",
          }
        );
    ...
    const getPersonalInfo = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}/alumni/${username}`,
          {
            credentials: "include",
          }
        );
        ...
        if (res.status === 403) {
          ...
          throw new Error(
            `Failed to fetch personal info: ${res.status} - ${res.statusText}`
          );
        }
      } catch (error) {
    ...
    const fetchBGPicture = async () => {
      try {
        ...
        if (res.status === 403) {
          ...
          throw new Error(
            `Failed to fetch cover picture: ${res.status} - ${res.statusText}`
          );
        }
      } catch (error) {
  ...
  }, [username, userId]); // Run only when username or userId changes
  ...
  const fetchExperiances = async () => {
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/experiences/${userId}`,
        {
          credentials: "include",
        }
      );
  ...
  const fetchEducations = async () => {
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/education/${userId}`,
        {
          credentials: "include",
        }
      );
      ...
      if (res.status === 403) {
        ...
        throw new Error(
          `Failed to fetch education data. Status: ${res.status}`
        );
      }
    } catch (error) {
  ...
  const deleteExperiance = async (id) => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/experiences/${id}`,
        {
          method: "DELETE",
          headers: {
            "Content-type": "application/json",
      ...
      if (res.status === 403) {
        ...
        setExperiances(
          experiances.filter((experiance) => experiance.id !== id)
        );
        window.location.reload();
      }
    } catch (error) {
  ...
  const getExperienceById = async (id) => {
    const desiredExperience = experiances.find(
  ...
  const updateEducation = async (
    ...
    try {
      setloading(true);
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/education`,
        {
          method: "PUT",
          headers: {
            "Content-type": "application/json",
  ...
  const deleteEducation = async (id) => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/education/${id}`,
        {
          method: "DELETE",
          headers: {
            "Content-type": "application/json",
  ...
  const submitFile = async () => {
    ...
    try {
      ...
      const response2 = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/check-username`,
        {
          method: "POST",
          credentials: "include",
          headers: {
      ...
      if (responseusername.isUsernameTaken) {
        ...
        try {
          const res = await fetch(
            `${import.meta.env.VITE_BACKEND_URL}/alumni`,
            {
              method: "PUT",
              credentials: "include",
              headers: {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      setloading(true);
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/uploadProfilePicture`,
        {
          method: "POST",
          credentials: "include",
  ...
  const handleUploadCover = async () => {
    ...
    try {
      setloading(true);
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/uploadCoverPicture`,
        {
          method: "POST",
          credentials: "include",
          body: formData,
        ...
        <section className="personal personal_onprofile">
          <div className="personal_post">
            <section className="personal_info onprofile bgc_overlay">
              ...
              <div className="profileImageHolder toshow">
                <img
                  className={
                    placeholders?.[0]?.role === "Student"
                      ? "profileImage"
              ...
              <ul>
                {placeholders?.length > 0 ? (
                  <>
                      ...
                      <div>
                        ...
                        <div className="edit_save_icon">
                          {showEditpersonalinfoPopup ? (
                            ...
                            <FaPen
                              onClick={() => setshowEditpersonalinfoPopup(true)}
                            />
                          )}
                        </div>
                      </div>
                      <Button
                        onClick={() => settranscriptPopUp(true)}
                    ...
                    {transcriptPopUp && (
                      <Transcript
                        close={() => settranscriptPopUp(false)}
                        email={placeholders[0].email}
                      />
                    )}
                    <li>
                      {experiances && (
                        <p className="work_place">{stillWorking()}</p>
                      )}
          ...
          <div className="learning_place">
            ...
            <section className=" personal_info add_story onprofile">
              ...
              {activities?.length > 0 && (
                <div className="see_all_posts">
                  <Link to={`/post/${username}?source=edit`}>
                    See all Posts
                  </Link>
                </div>
              )}
            ...
            <section className="experiance_and_education e_and_e_on_Profile">
              <section className="personal_info a">
                ...
                {showAddExperiance && (
                  <AddExperiance
                    loading={loading}
                    updateExperiance={updateExperiance}
                    showEditExperiance={showEditExperiance}
                    experiances={experiance}
                    onAdd={addExperiance}
                    showAddExperiance={setshowAddExperiance}
                  />
                )}
