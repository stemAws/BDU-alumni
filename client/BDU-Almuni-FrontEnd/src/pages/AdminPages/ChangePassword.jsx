...
const ChangePassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setloading(true);
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/check-password/${personId}`,
        {
          method: "POST",
          headers: {
      ...
      if (!responsePassword.passwordExists) {
        ...
        try {
          const res = await fetch(
            `${import.meta.env.VITE_BACKEND_URL}/change-password/${personId}`,
            {
              method: "PUT",
              headers: {
                "Content-Type": "application/json",
    ...
    <div className="chang_pass">
      ...
      <div className="change_pasword">
        ...
        <form className="change_pasword_form" onSubmit={handleSubmit}>
          ...
          <div>
            <Button
              className={"change_passwordbtn"}
              type={"submit"}
              disabled={loading}
              text={loading ? "Changing..." : "Change"}
            />
          </div>
        </form>
      </div>
