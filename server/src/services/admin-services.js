...
exports.fetchSuggestedPostsToAdmin = async () => {
  try {
    const [result] = await db.query('SELECT * FROM posts WHERE suggestToAdmin = 1');
    return result;
  } catch (error) {
    throw new Error('Error fetching suggested posts to admin: ' + error.message);
  }
};
...
exports.fetchSuggestedPostsByAdmin = async () => {
  try {
    const [result] = await db.query('SELECT * FROM posts WHERE suggestedByAdmin = 1');
    return result;
  } catch (error) {
    throw new Error('Error fetching suggested posts by admin: ' + error.message);
  }
};

exports.fetchGeoData = async () => {
  try {
    const [rows, fields] = await db.query('SELECT address, COUNT(*) AS count FROM alumni GROUP BY address');
...
exports.addDonation = async (title, link, description) => {
  try {
    ...
    if (donationResult.insertId) {
      return { message: 'Donation added successfully', donationId: donationResult.insertId };
    } else {
