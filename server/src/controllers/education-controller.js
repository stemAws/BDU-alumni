...
exports.addEducation = async (req, res) => {
  try {
    ...
    const affectedRows = await educationService.addEducation(
      req.body,
      alumniId
    );
    res
      .status(201)
      .json({ message: "Education added successfully", affectedRows });
  } catch (error) {
...
exports.updateEducation = async (req, res) => {
  try {
    ...
    if (affectedRows === 0)
      ...
      res
        .status(200)
        .json({ message: "Education updated successfully", affectedRows });
  } catch (error) {
...
exports.deleteEducationById = async (req, res) => {
  try {
    ...
    if (affectedRows === 0)
      res.status(404).json({ error: "No record found by the given id" });
    else res.status(200).json({ message: "Education deleted successfully" });
  } catch (error) {
