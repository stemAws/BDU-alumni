...
exports.addExperience = async (req, res) => {
  try {
    ...
    const affectedRows = await experienceService.addExperience(
      req.body,
      alumniId
    );
    if (affectedRows > 0) {
...
exports.updateExperience = async (req, res) => {
  try {
    ...
    if (affectedRows > 0) {
      ...
      res.status(404).json({
        success: false,
        message: "Experience not found or not updated",
      });
    }
  } catch (error) {
...
exports.getExperience = async (req, res) => {
  try {
    ...
    if (experience) {
      ...
      res.status(404).json({
        success: false,
        error: "No record found for the given ID or username",
      });
    }
  } catch (error) {
...
exports.deleteExperienceById = async (req, res) => {
  try {
    const affectedRows = await experienceService.deleteExperience(
    ...
    if (affectedRows > 0) {
      ...
      res
        .status(404)
        .json({ success: false, message: "No record found for the given ID" });
    }
  } catch (error) {
