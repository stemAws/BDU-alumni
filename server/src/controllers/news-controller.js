...
exports.searchNews = async (req, res) => {
  try {
    ...
    const events = await newsService.searchNewsBy(title, category);
    res.send(events);
  } catch (error) {
...
exports.updateNews = async (req, res) => {
  try {
    ...
    const affectedRows = await newsService.updateANews(
      newsId,
      updatedNewstData
    );
    ...
    if (affectedRows === 0) {
      return res
        .status(404)
        .json({ error: `No record with the given id: ${newsId}` });
    }
